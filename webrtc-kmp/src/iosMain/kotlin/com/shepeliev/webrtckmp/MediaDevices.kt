...
private object MediaDevicesImpl : MediaDevices {
    override suspend fun getUserMedia(
        ...
        val audioTrack =
            constraints.audio?.let { audioConstraints ->
                ...
                val track =
                    WebRtc.peerConnectionFactory.audioTrackWithSource(
                        source = audioSource,
        ...
        val videoTrack =
            constraints.video?.let { videoConstraints ->
                ...
                val iosVideoTrack =
                    WebRtc.peerConnectionFactory.videoTrackWithSource(
                        source = videoSource,
                        trackId = NSUUID.UUID().UUIDString(),
                    )
                val videoCaptureController =
                    CameraVideoCapturerController(
    ...
    override suspend fun enumerateDevices(): List<MediaDeviceInfo> {
        val captureDevices =
            RTCCameraVideoCapturer.captureDevices().map {
